...
aws ecr get-login-password --region eu-west-3 --profile operations | docker login --username AWS --password-stdin 763104351884.dkr.ecr.eu-west-3.amazonaws.com

# build docker image (if running on M1/M2 --> specify platform with : --platform linux/amd64)
docker build -t training-image -f images/train/Dockerfile --platform linux/amd64 .

# Login to docker registry
aws ecr get-login-password --region eu-west-3 --profile operations | docker login --username AWS --password-stdin $operations.dkr.ecr.eu-west-3.amazonaws.com
